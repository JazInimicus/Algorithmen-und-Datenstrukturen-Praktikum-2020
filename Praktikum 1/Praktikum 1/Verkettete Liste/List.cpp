...
*/
	Node* iterator = head_tail->next;
	while (iterator != head_tail) {
...
*/
	...
	Node* node = new Node(key, head_tail->next, head_tail);
	head_tail->next->prev = node;
	head_tail->next = node;
	list_size++;
...
*/
	Node* node = new Node(key, head_tail, head_tail->prev);

	head_tail->prev->next = node;
	head_tail->prev = node;
	list_size++;
}
